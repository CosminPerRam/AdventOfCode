...
//this part is exactly the same as the first one, the only thing changed is the process_command
//it now takes all characters at once and puts them in the same order into the other stack (with a buffer)
...
fn process_command(stacks: &mut Vec<VecDeque<char>>, count: usize, from: usize, to: usize) {
    ...
    for _ in 0..count {
        ...
        buffer.push_front(character);
    }

    for _ in 0..count {
        let character = *buffer.front().unwrap();
